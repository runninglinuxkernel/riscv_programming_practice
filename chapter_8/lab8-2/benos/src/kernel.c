 ...
 *
 * 实现一个小的memcpy汇编函数
 * 从0x20000地址拷贝32字节到0x2A000地址处，并使用gdb来比较数据是否拷贝正确
 */
...
{
	my_memcpy_asm_test1(0x20000, 0x27000, 32);
    my_memcpy_asm_test2(0x20000, 0x27000, 32);
	/*lab7-3: memset函数实现*/
	memset((void *)0x27002, 0x55, 48);
	/*lab7-4: 使用内嵌汇编与宏的结合*/
	my_ops_test();
	/*lab7-5*/
	test_sysregs();
...
void create_illegal_intr(void)
{
	...
	//*p = 0xbadbeef; // 这并非illegal inst, 它是"jal	t4,104eca"
	*p = 0x300027f3; // csrr    a5,mstatus, S模式下访问mstatus会发生illegal inst异常
...
void kernel_main(void)
{
    ...
    /* 访问0x20001并不会导致Load address misaligned
     * 但是访问0x100001就会导致Load address misaligned
     * D1S硬件支持非对齐的访问,
     * 但是对于未实现的地址的非对齐访问(比如0x100001)就会导致异常
     */
	printk("addr 0x20001's val = 0x%x\n", *((unsigned int *)0x20001));
