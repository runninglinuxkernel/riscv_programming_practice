...
{
	unsigned long reg = PLIC_PRIORITY(hwirq);
...
void plic_enable_irq(int cpu, int hwirq, int enable)
{
	...
	int hart = CPU_TO_S_HART(cpu);
	unsigned long reg = PLIC_MENABLE(hart) + 4*(hwirq / 32);
...
void plic_enable_irq_for_hart(int hart, int hwirq, int enable)
{
	unsigned int hwirq_mask = 1 << (hwirq % 32);
	unsigned long reg = PLIC_MENABLE(hart) + 4*(hwirq / 32);
	unsigned long reg_ip = PLIC_PENDING(hwirq);
	...
	{
        /* 先清除中断 */
        writel(readl(reg_ip) & ~hwirq_mask, reg_ip);
...
void plic_handle_irq(struct pt_regs *regs)
{
	...
	int hart = CPU_TO_S_HART(0);

	unsigned long claim_reg = PLIC_MCLAIM(hart);

    printk("plic_handle_irq\n");
...
int plic_init(void)
{
	...
	for (i = 0; i < MAX_CPUS; i++) {
		/* 设置所有M/S模式处理器核心的中断优先级阈值为0*/
		//writel(0, PLIC_MTHRESHOLD(CPU_TO_HART(i)));
		writel(0, PLIC_MTHRESHOLD(CPU_TO_M_HART(i)));
        writel(0, PLIC_MTHRESHOLD(CPU_TO_S_HART(i)));

		for (hwirq = 1; hwirq <= MAX_PLIC_IRQS; hwirq++) {
			/*关闭PLIC中所有外部中断*/
			// plic_enable_irq(i, hwirq, 0);
			plic_enable_irq_for_hart(CPU_TO_M_HART(i), hwirq, 0);
			plic_enable_irq_for_hart(CPU_TO_S_HART(i), hwirq, 0);
